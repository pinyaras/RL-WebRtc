...
# client.create_database('RTC')
# switch to the created database
# client.switch_database('RTC')
...
async def offer(request):
    ...
    def on_datachannel(channel: RTCDataChannel):
        ...
        def h264_config_bitrate_at_fps(fps, bitrate):
            ...
            print("h264 config", bitrate * aiortc.codecs.h264.MAX_FRAME_RATE / fps)
        ...
        async def sendStats():
            ...
            frame_delay = stats_latest_frame_time - stats_last_frame_time
            ...
            # try:
            #     existing_df = pd.read_json('message.json', orient='index')
            ...
            # # save the appended dataframe to the json file
            # existing_df.to_json('RTCtraces.json',orient='index')

            # convert json message to InfluxDB format
            message = {
                # "Codec": encoder_name,
                # "Target FPS": str(reduced_video_track.target_fps),
                "Current FPS": float(fps),
                # "Target Resolution": str(reduced_video_track.target_height) + 'p',
                "estimate_bw":  float(video_sender.lastBitrateEstimate) / 1000 if hasattr(video_sender, "lastBitrateEstimate") else float(target_bitrate) / 1000,
                "target_bitrate": float(target_bitrate) / 1000,
                "fpsTarget": float(video_sender._RTCRtpSender__encoder.target_bitrate) / 1000,
                "current_bitrate": current_bps * 8 / 1000,
                "frame_delay": float(frame_delay)
            }

            json_body = [
                {
                    "measurement": "outbound-rtp",
                    "tags": {"encoder": "H264"},
            ...
            # append the json message to the existing measurement
            # client.write_points(json_body)
            # write_api.write("my-bucket", org, json_body)
            # write_api.write("webrtc", org, json_body)

            # with open('send_message.json', 'w') as f:
            #     json.dump(message, f)

            # # Append the new dictionary to the list in the file
            # with open('send_message.json', 'a+') as f:
            #     f.seek(0)  # Move the file pointer to the beginning
            #     try:
            #         data = json.load(f)  # Try to read the existing contents of the file
            #     except json.decoder.JSONDecodeError:
            #         data = []  # If the file is empty or invalid, initialize an empty list
            #     data.append(message)  # Append the new dictionary to the list
            #     f.seek(0)  # Move the file pointer to the beginning
            #     json.dump(data, f)  # Write the updated list to the file
            # append new dictionary to existing list
            try:
                with open('send_message.json', "r") as f:
                    data = json.load(f)
            except json.decoder.JSONDecodeError:
                data = []
        ...
        async def on_message(message):
            ...
            if isinstance(message, str) and message.startswith("ping"):
                ...
                pkt_delay = float(clock.current_datetime().timestamp()*1000 - time)
                channel.send("pong" + message[4:])
                stat = await video_sender.getStats()

                remote_inbound_rtp_values = [item for item in stat.values() if item.type == 'remote-inbound-rtp']
                result = {}
                for value in remote_inbound_rtp_values:
                    result['packetsReceived'] = value.packetsReceived
                    result['packetsLost'] = value.packetsLost
                    result['roundTripTime'] = value.roundTripTime
                    result['jitter'] = value.jitter / 1000
            ...
            if isinstance(message, str) and message.startswith("target_height"):
                try:
                    ...
                    logging.error(e)
            if not RL:
                if hasattr(video_sender, "lastBitrateEstimate"):
                    print("Last BW EST", video_sender.lastBitrateEstimate)
                    video_sender._RTCRtpSender__encoder.target_bitrate = h264_config_bitrate_at_fps(
                        reduced_video_track.target_fps, target_bitrate)

                    video_sender._RTCRtpSender__encoder.target_bitrate =     video_sender.lastBitrateEstimate
                    print("New target bitrate",video_sender._RTCRtpSender__encoder.target_bitrate)
            ...
            try:
                with open('recv_message.json', "r") as f:
                    data = json.load(f)
            except json.decoder.JSONDecodeError:
                data = []

            data.append(recv_message)

            # write updated data to JSON file
            with open('recv_message.json', 'w') as f:
                json.dump(data, f, indent=4)

            recv_json_body = [
                {
                    "measurement": "recv_pkt",
                    "tags": {"encoder": "H264"},
            ...
            # client.write_points(json_body)
            # write_api.write("my-bucket", org, json_body)
            # write_api.write("webrtc", org, recv_json_body)
            
            if RL:
                file_name = 'rl_bitrate.json'
                # Load the existing JSON file
                with open(file_name, 'r') as f:
                    data = json.load(f)

                # video_sender._RTCRtpSender__encoder.target_bitrate = data['rl_bitrate']
                video_sender._RTCRtpSender__encoder.target_bitrate = h264_config_bitrate_at_fps(
                        reduced_video_track.target_fps, target_bitrate)
                print(data['rl_bitrate'])
...
if __name__ == "__main__":
    ...
    # parser.add_argument("--RL", type=bool , default=False, help="Read the media from a file and sent it.")
